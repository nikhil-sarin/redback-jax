...
class Spectrum:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.flux_density_err = jnp.asarray(flux_density_err) if flux_density_err is not None else None
        self.time = time
        self.name = name
        ...
        if len(self.wavelength) != len(self.flux_density):
            raise ValueError("Wavelength and flux_density arrays must have the same length")
        
        if self.flux_density_err is not None and len(self.flux_density_err) != len(self.flux_density):
            raise ValueError("flux_density_err must have the same length as flux_density")
    ...
    def plot_data(
        ...
        **kwargs
        ...
        if show_errors and self.flux_density_err is not None:
            ...
            axes.errorbar(wavelength, flux_density, yerr=flux_err, 
...
class Transient:
        ...
        **kwargs
        ...
        if y_col is None:
            ...
            if y_col not in data.columns:
                ...
                if not found:
                    raise ValueError(f"Could not find {data_mode} column in data. "
        ...
        time_err = data[time_err_col].values if time_err_col and time_err_col in data.columns else None
        y_err = data[y_err_col].values if y_err_col and y_err_col in data.columns else None
    ...
    def plot_data(
        ...
        **kwargs
        ...
        if self.time is None:
            raise ValueError("No data loaded. Use from_data_file() or provide data in constructor.")
        ...
        if show_errors and self.y_err is not None:
            ...
            axes.errorbar(time, y, yerr=y_err, fmt='o', 
                         color=color, alpha=alpha, **kwargs)
        else:
    ...
    def plot_model(
        ...
        **kwargs
        ...
        # Plot model
        axes.plot(np.asarray(t_model), np.asarray(y_model), 
    ...
    def __repr__(self) -> str:
        ...
        return (f"Transient(name='{self.name}', data_mode='{self.data_mode}', "
