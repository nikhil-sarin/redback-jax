    ...
    **kwargs
    ...
    for i, band in enumerate(unique_bands):
        ...
        if y_err_band is not None:
            plot_errorbar_jax(ax, time_band, y_band, yerr=y_err_band, 
                            fmt='o', color=colors[i], label=f'{band} data')
        else:
            ax.scatter(np.asarray(time_band), np.asarray(y_band), 
                      color=colors[i], label=f'{band} data')
        ...
        if model_func is not None and model_params is not None:
            ...
            ax.plot(np.asarray(t_model), np.asarray(y_model), 
                   color=colors[i], alpha=0.8, label=f'{band} model')
