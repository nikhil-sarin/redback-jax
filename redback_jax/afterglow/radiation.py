...
def forward_shock_state(
    ...
    sound_speed = speed_of_light * jnp.sqrt(
        adiabatic_index * (adiabatic_index - 1.0) * gamma_minus_one
    ...
    log10_radius_base = (
        jnp.log10(3.0 - density_index) + log10_electrons
        - math.log10(_FOUR_PI) - log10_density
    ) / (3.0 - density_index)
    ...
    radius = jnp.cumsum(jnp.where(expansion, radius_increment * relative_expansion,
                                  radius_increment))

    log10_local_density = log10_density - density_index * jnp.log10(radius)
    shock_factor = ((adiabatic_index * gamma + 1.0)
                    / (adiabatic_index - 1.0)) * gamma_minus_one
    log10_magnetic_field = 0.5 * (
        math.log10(8.0 * math.pi) + jnp.log10(epsilon_b)
        + log10_local_density + math.log10(proton_mass)
        + 2.0 * math.log10(speed_of_light) + jnp.log10(shock_factor)
    )
    log10_gamma_maximum = 0.5 * (
        math.log10(1.5 * _FOUR_PI * qe / sigma_T) - log10_magnetic_field
    )
    log10_base_gamma = (
        jnp.log10(epsilon_e / accelerated_fraction) + jnp.log10(gamma_minus_one)
        + math.log10(proton_mass / electron_mass)
    )

    def above_two(_):
        return log10_base_gamma + jnp.log10((electron_index - 2.0) / (electron_index - 1.0))

    def equal_two(_):
        coefficient = 1.0 / (
            math.log(10.0) * (log10_gamma_maximum - log10_base_gamma)
        )
    ...
    log10_gamma_minimum = lax.cond(
        electron_index > 2.0, above_two,
        lambda _: lax.cond(electron_index == 2.0, equal_two, below_two, None), None)
    log10_nu_minimum_prime = (
        jnp.log10(3.0 * spectral_peak * qe / (_FOUR_PI * electron_mass * speed_of_light))
        + 2.0 * log10_gamma_minimum + log10_magnetic_field
    )
    log10_peak_power = (
    ...
    log10_electron_energy = (
        log10_gamma_minimum + math.log10(electron_mass * speed_of_light**2)
    )
    return ShockState(beta, log10_electrons, solid_angle, radius,
                      log10_magnetic_field, log10_gamma_minimum,
                      log10_nu_minimum_prime, log10_peak_power,
                      log10_electron_energy)


@jit
def observer_state(log10_distance, initial_solid_angle, azimuth_step,
                   observer_angle_value, gamma, shock):
    ...
    log10_gamma_cooling = (
        ...
        - jnp.log10(gamma) - 2.0 * shock.log10_magnetic_field
        - jnp.log10(on_axis_time)
    )
    log10_nu_cooling = (
        log10_doppler + math.log10(0.286 * 3.0 * qe / (_FOUR_PI * electron_mass * speed_of_light))
        + 2.0 * log10_gamma_cooling + shock.log10_magnetic_field
    )
    log10_nu_minimum = log10_doppler + shock.log10_nu_minimum_prime
    log10_peak_flux = (
        jnp.log10(initial_solid_angle) + shock.log10_electrons
        - math.log10(_FOUR_PI) + shock.log10_peak_power + 3.0 * log10_doppler
        - math.log10(_FOUR_PI) - 2.0 * log10_distance
    )
    ...
    log10_blackbody_factor = (
        jnp.log10(2.0 * visible_solid_angle * cosine_edge) + log10_doppler
        + shock.log10_electron_energy + 2.0 * jnp.log10(shock.radius)
        - 2.0 * math.log10(speed_of_light) - 2.0 * log10_distance
    )
    return ObserverState(log10_blackbody_factor, log10_peak_flux,
                         log10_nu_cooling, log10_nu_minimum, observer_time)
...
def synchrotron_log_flux(observer, log10_frequency, electron_index):
    ...
    fast_high = (observer.log10_peak_flux - 0.5 * (log_num - log_nuc)
                 - 0.5 * electron_index * (log10_frequency - log_num))
    slow_low = observer.log10_peak_flux + (log10_frequency - log_num) / 3.0
    slow_mid = (observer.log10_peak_flux
                - 0.5 * (electron_index - 1.0) * (log10_frequency - log_num))
    slow_high = (observer.log10_peak_flux
                 - 0.5 * (electron_index - 1.0) * (log_nuc - log_num)
                 - 0.5 * electron_index * (log10_frequency - log_nuc))
    fast_flux = jnp.where(log10_frequency < log_nuc, fast_low,
                          jnp.where(log10_frequency < log_num, fast_mid, fast_high))
    slow_flux = jnp.where(log10_frequency < log_num, slow_low,
                          jnp.where(log10_frequency < log_nuc, slow_mid, slow_high))
    optically_thick = (
        observer.log10_blackbody_factor + 2.0 * log10_frequency
