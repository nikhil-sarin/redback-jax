...
def example_multi_band_transient():
    ...
    for band in bands:
        ...
        ax.errorbar(np.asarray(time_band), np.asarray(mag_band), 
                   yerr=np.asarray(err_band), fmt='o', 
                   color=colors[band], label=f'{band} band', alpha=0.7)
...
def example_spectrum():
    ...
    print(f"Wavelength range: {float(jnp.min(spectrum.wavelength)):.0f} - {float(jnp.max(spectrum.wavelength)):.0f} Å")
...
2.0,2.2,0.10,i"""
    ...
    try:
        ...
        for band in np.unique(transient.bands):
            ...
            ax.errorbar(np.asarray(time_band), np.asarray(flux_band),
                       yerr=np.asarray(err_band), fmt='o',
                       color=colors[band], label=f'{band} band', alpha=0.7)
